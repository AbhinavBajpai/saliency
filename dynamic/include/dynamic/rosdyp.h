...
class dySubscriber 
{
  public:
	...
	bool stop;
/*
	dySubscriber& operator=(dySubscriber& a){
		dySubscriber<M> p;
		p.data_class = a.data_class;
		p.name = a.name;
		p.UID = a.name;
		p.nh = a.nh;
		p.lockable = a.lockable;
		p.first = a.first;
		p.second = a.second;
		p.change = a.change;
		p.stop = a.stop;
...
*/
	void swap(dySubscriber* a){
		this->data_class = a->data_class;
		this->name = a->name;
		this->UID = a->name;
		this->nh = a->nh;
		this->lockable = a->lockable;
		this->first = a->first;
	...
	void subscribe(const boost::function<void (const boost::shared_ptr<M const>&)>& callback, const VoidConstPtr tracked_object)
{
	while (ros::ok()){
		test = checkForTopic();
		first = true;
		...
		if(test){
			while(ros::ok() and second){
				subg = nh.subscribe(topic, 1000, callback, tracked_object);
				first=true;
				while(ros::ok() and first and second){
					test = checkForTopic();
					if (change) {
