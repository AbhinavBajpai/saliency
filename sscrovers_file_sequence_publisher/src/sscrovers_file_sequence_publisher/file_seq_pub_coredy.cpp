    ...
    it_(*_n)
{
  ...
  //add '/' when missed at the end of path
  ...
  image_dp = dyPublisher<sensor_msgs::Image>("cam_image", *_n);
  odom_dp  = dyPublisher<nav_msgs::Odometry>("odom", *_n);
...
void FileSeqPubCore::publishImage()
{
  ...
  image_dp.publishOnceWait(*(cv_img_ptr_->toImageMsg()));
}
...
void FileSeqPubCore::publishOdom()
{
  ...
  odom_dp.publishOnceWait(odom_msg_);
}
...
int main(int argc, char **argv)
{
  ...
  file_pub_core->odom_dp.onShutdown();
  file_pub_core->image_dp.onShutdown();
